...
const WALL_MATERIALS: Record<string, { hex: string, roughness: number, metalness: number }> = {
  'aerogel': { hex: '#e2e8f0', roughness: 0.9, metalness: 0.1 },
  'pir': { hex: '#cbd5e1', roughness: 0.8, metalness: 0.05 },
  'eps': { hex: '#c8c2b7', roughness: 0.85, metalness: 0.1 },
  'hempcrete': { hex: '#bbaea0', roughness: 0.95, metalness: 0.0 },
  'ceb': { hex: '#9a7b4f', roughness: 0.98, metalness: 0.0 },
  'hollow_polymer': { hex: '#48525b', roughness: 0.6, metalness: 0.2 },
};

const ROOF_MATERIALS: Record<string, { hex: string, roughness: number, metalness: number }> = {
  'low_e_alu': { hex: '#f8fafc', roughness: 0.1, metalness: 0.95 },
  'cool_roof': { hex: '#ffffff', roughness: 0.3, metalness: 0.1 },
  'galvanized': { hex: '#94a3b8', roughness: 0.45, metalness: 0.9 },
  'terracotta': { hex: '#c2410c', roughness: 0.7, metalness: 0.0 },
  'green_roof': { hex: '#4ade80', roughness: 0.9, metalness: 0.0 },
  'solar_absorbent': { hex: '#1e293b', roughness: 0.7, metalness: 0.1 },
};
...
export function buildEmergencyShelter(config: ShelterConfig): BuiltShelterModel {
  ...
  // Map material IDs to physics
  const wallMatData = WALL_MATERIALS[config.wallMaterialId] || WALL_MATERIALS['eps'];
  const roofMatData = ROOF_MATERIALS[config.roofMaterialId] || ROOF_MATERIALS['galvanized'];

  // 1. Generate PBR Textures
  const roofTextures = generateCorrugatedRoofTextures(config.weathering);
  const wallTextures = generateWallPanelTextures(wallMatData.hex, config.weathering);
  ...
  function updateConfig(cfg: ShelterConfig) {
    ...
    // Material update
    const wallMatData = WALL_MATERIALS[cfg.wallMaterialId] || WALL_MATERIALS['eps'];
    const roofMatData = ROOF_MATERIALS[cfg.roofMaterialId] || ROOF_MATERIALS['galvanized'];
    
    wallMaterial.color.set(wallMatData.hex);
    wallMaterial.roughness = wallMatData.roughness;
    wallMaterial.metalness = wallMatData.metalness;

    roofMaterial.color.set(roofMatData.hex);
    roofMaterial.roughness = roofMatData.roughness;
    roofMaterial.metalness = roofMatData.metalness;
  }
