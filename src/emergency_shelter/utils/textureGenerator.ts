...
export function generateWallPanelTextures(baseHex = '#c8c2b7', weathering = 0.15) {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const idx = (y * size + x) * 4;
      const noise = (pseudoNoise(x, y) - 0.5) * 15;
      // Ambient dirt gradient near the bottom base chassis
      const baseDirt = Math.pow(y / size, 3) * weathering * 55;
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      // Micro orange peel noise and deeper scratches for realism
      const micro = (pseudoNoise(x * 5, y * 5) - 0.5) * 0.18;
      nx += micro;
      ny += micro;
