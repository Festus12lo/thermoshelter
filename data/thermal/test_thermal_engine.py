#!/usr/bin/env python3
"""
Unit tests for V1 Thermal Physics Engine
Tests all specified test cases from physics_spec_prompt.txt
"""
...
def create_test_weather_data(hours=24, temp_base=10.0, temp_amplitude=5.0, solar_base=200.0):
    """Create synthetic test weather data for controlled testing."""
    import math
    weather_data = []
    for hour in range(hours):
        # Simple diurnal cycle
        temp = temp_base + temp_amplitude * math.sin(2 * math.pi * hour / 24)
        solar = max(0, solar_base * math.sin(2 * math.pi * (hour - 6) / 24)) if 6 <= hour <= 18 else 0

        weather_data.append({
            'datetime': f'2026-01-01 {hour:02d}:00:00',
            'temperature_2m': temp,
...
def test_3_thin_vs_thick_walls():
    ...
    try:
        ...
        if thick_wall_heat < thin_wall_heat:
            print("  ✓ Thicker walls show lower heat flow (better insulation)")
        else:
            print("  ⚠ Thicker walls do not show lower heat flow - investigating...")
...
def test_4_insulation_vs_metal():
    ...
    try:
        ...
        if met_wall_heat > ins_wall_heat * 2:  # Expect at least 2x difference
            print("  ✓ Metal shows significantly higher heat flow (poor insulator) as expected")
        else:
            print("  ⚠ Difference in heat flow less than expected - checking material properties...")
...
def test_5_no_vs_significant_solar():
    ...
    try:
        ...
        if high_solar_avg > no_solar_avg:
            print("  ✓ Higher solar radiation leads to higher indoor temperatures")
        else:
            print("  ⚠ Expected temperature increase with solar not observed - checking...")
...
def test_6_low_vs_high_ventilation():
    ...
    try:
        ...
        if high_vent_flow > low_vent_flow:
            print("  ✓ Higher ventilation level produces greater heat exchange")
        else:
            print("  ⚠ Ventilation heat flow not higher as expected")
        ...
        if high_temp_range <= low_temp_range:
            print("  ✓ High ventilation reduces indoor temperature swing (better coupling)")
        else:
            print("  ⚠ High ventilation does not reduce temperature swing as expected")
...
def test_7_different_thermal_mass():
    ...
    try:
        ...
        if high_max_rate < low_max_rate:
            print("  ✓ High thermal mass shows slower temperature response")
        else:
            print("  ⚠ Expected slower response with high thermal mass not observed")
        ...
        if high_capacitance > low_capacitance:
            print("  ✓ High thermal mass configuration has greater heat capacity")
        else:
            print("  ⚠ Heat capacity not higher as expected for high mass materials")
...
def test_8_material_property_lookup_failure():
    ...
    try:
        ...
        print("  ✗ Test 8 FAILED: Should have raised ValueError for invalid material ID")
        return False
    except ValueError as e:
        if "Material ID not found" in str(e):
            print("  ✓ Correctly raised ValueError for invalid material ID")
        ...
        print(f"  ✗ Test 8 FAILED: Unexpected exception type: {e}")
        return False
...
def test_9_metal_wall_numerical_stability():
    ...
    try:
        ...
        print("  ✓ Metal wall numerical stability verified: no Inf, no NaN, no numerical overflow")
        print("  ✓ Test 9 PASSED\n")
        return True
...
def test_10_geometry_opening_validation():
    ...
    try:
        ...
        try:
            ...
            print("  ✗ Test 10 FAILED: Should have raised ValueError for negative window area")
            return False
        except ValueError:
            pass
        ...
        try:
            ...
            print("  ✗ Test 10 FAILED: Should have raised ValueError for opening area > gross wall area")
            return False
        except ValueError:
            pass

        print("  ✓ Correctly validated opening bounds and rejected invalid geometry")
...
def test_11_r_value_u_value_diagnostics():
    ...
    try:
        ...
        print(f"  Wall R-value: {wall_r:.3f} m²K/W, U-value: {wall_u:.3f} W/(m²K)")
        print(f"  Roof R-value: {roof_r:.3f} m²K/W, U-value: {roof_u:.3f} W/(m²K)")
        print("  ✓ R-value and U-value diagnostics verified")
...
def test_12_thermal_capacitance_transparency():
    ...
    try:
        ...
        print("  ✓ Thermal capacitance C = ρ * V * cp verified across all components")
...
def test_13_energy_balance():
    ...
    try:
        ...
        print("  ✓ Q_net matches sum of heat flow components within 1e-5 W tolerance for 24 hours")
...
def test_14_radiation_numerical_safety():
    ...
    try:
        ...
        try:
            ...
            print("  ✗ Test 14 FAILED: Should have raised ValueError for NaN temperature")
            return False
        except ValueError:
            pass
        ...
        try:
            ...
            print("  ✗ Test 14 FAILED: Should have raised ValueError for Inf temperature")
            return False
        except ValueError:
            pass

        print("  ✓ Correctly rejected NaN and Inf inputs for longwave radiation")
...
def test_15_thermal_time_constant():
    ...
    try:
        ...
        print("  ✓ Thermal time constant and stability audit verified")
...
def test_17_shimla_comparison():
    ...
    try:
        ...
        print("  ✓ Leh vs Shimla mountain climate comparison verified")
...
def test_19_karur_climate_comparison():
    ...
    try:
        ...
        print("  ✓ Jaipur vs Karur warm climate comparison verified")
...
def test_20_material_comparison():
    ...
    try:
        ...
        for res in results:
            print(f"    {res['material_id']:12s} | U: {res['wall_u_value_W_m2K']:5.2f} W/m²K | Avg Tin: {res['avg_indoor_temp_C']:5.1f}°C | Avg Wall Heat: {res['avg_wall_heat_flow_W']:6.1f} W")

        print("  ✓ Controlled material comparison utility verified: physical ranking holds")
